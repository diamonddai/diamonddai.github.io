...
import { Slider, ConfigProvider } from 'antd'; // 引入 Slider 组件
...
const ForceGraph = ({ nodeId, labels, filters, dateRange, interval, selectedLabel }) => {
  ...
  useEffect(() => {
    ...
    node.filter(d => d.id === nodeId).each(function(d) {
      ...
      drawPolarAreaChart(d3.select(this), center, tooltip, labels, nodeId, selectedLabel);

      drawRingChart(d3.select(this), firstRing, 75, 65, tooltip, labels, nodeId, selectedLabel);

      drawRingChart(d3.select(this), secondRing, 88, 78, tooltip, labels, nodeId, selectedLabel);
    });
...
const drawPolarAreaChart = (container, scores, tooltip, labels, nodeId, selectedLabel) => {
  const radius = 50;
  const baseColor = getLabelColor(nodeId, labels);
  const color = baseColor === 'grey' ? d3.scaleSequential(d3.interpolateGreys).domain([0, 1]) : d3.scaleLinear().domain([0, 1]).range(['#ffffff',baseColor]);
  ...
  const selectedLabelIndex = selectedLabel ? labelMapping.indexOf(selectedLabel) : null;
  ...
  container.append('g')
    ...
    .attr('d', arcGenerator)
    .attr('fill', d => {
      if (selectedLabelIndex !== null && d.index === selectedLabelIndex) {
        return d3.interpolate("#FFFFFF", getSelectedColor(selectedLabel))(d.score); // Gradient for the selected label
      } else {
        return d.score === 0 ? 'rgba(0, 0, 0, 0)' : color(d.score);
      }
    })
    .on('mouseover', function(event, d) {
      tooltip.transition().duration(200).style('opacity', .9);
...
const drawRingChart = (container, scores, outerRadius, innerRadius, tooltip, labels, nodeId, selectedLabel) => {
  const baseColor = getLabelColor(nodeId, labels);
  const color = baseColor === 'grey' ? d3.scaleSequential(d3.interpolateGreys).domain([0, 1]) : d3.scaleLinear().domain([0, 1]).range(['#ffffff',baseColor]);
