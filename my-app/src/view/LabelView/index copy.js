...
const LabelView = () => {
  ...
  useEffect(() => {
    ...
    axios.get('http://106.15.36.115:8008/initiallabels')
      .then(response => {
        ...
        setLabels(decodedLabels);
        setUserIds(Object.keys(decodedLabels))

        const selectedUserIds = Object.keys(labelsData);
        setCurrentUserId(selectedUserIds.length > 0 ? selectedUserIds[0]: null);
  
        // 加载 tweets.json 文件
        axios.get('http://127.0.0.1:3001/organizedbyuserid')
          .then(response => {
            setJsonData(response.data);

            // 根据用户 ID 筛选 jsonData 中的用户数据，只取每个用户的第一个帖子
            const loadedUsers = Object.values(response.data)
  ...
  useEffect(() => {
    ...
    if (Object.keys(activeNodes).length > 0) {
      ...
      console.log(labels,newLabelsArray, newUserIds)
